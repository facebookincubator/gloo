...
void HipAllreduceHalvingDoubling<T, W>::initBinaryBlocks() {
...
template <typename T, typename W>
HipAllreduceHalvingDoubling<T, W>::HipAllreduceHalvingDoubling(
...
template <typename T, typename W>
void HipAllreduceHalvingDoubling<T, W>::run() {
...
template <typename T, typename W>
void HipAllreduceHalvingDoubling<T, W>::devicePointerInit() {
...
template <typename T, typename W>
template <typename U>
void HipAllreduceHalvingDoubling<T, W>::init(
  ...
  if (bytes_ < kOnDeviceThreshold) {
    localReduceOp_ =
        hipHostReduce(streams_, devicePtrs_, scratch_, fn_, 0, count_);
    localBroadcastOp_ =
        hipHostBroadcast(streams_, devicePtrs_, scratch_, 0, count_);
  } else {
    localReduceOp_ =
        hipDeviceReduce(streams_, devicePtrs_, scratch_, fn_, 0, count_);
    localBroadcastOp_ =
        hipDeviceBroadcast(streams_, devicePtrs_, scratch_, 0, count_);
  }
}
...
void HipAllreduceHalvingDoubling<T, W>::init(
  ...
  {
    HipDeviceScope scope(scratch_.getDeviceID());
    // pad receive buffer size to nearest power of 2 to ensure sufficient space
    recvBuf_ = W::Pointer::alloc(chunkSize_ << steps_);
  }
...
template <typename T, typename W>
template <typename U>
void HipAllreduceHalvingDoubling<T, W>::initReductionsAndBroadcasts(
    typename std::enable_if<
        std::is_same<U, HipHostWorkspace<T>>::value,
        typename U::Pointer>::type*) {
  ...
  if (sendCounts_[0] * sizeof(T) < kOnDeviceThreshold) {
    if (!devicePtrsForFirstSend_.empty()) {
      reduceBeforeFirstSend_ = hipHostReduce(
          streams_,
    ...
    if (!devicePtrsForFirstRecv_.empty()) {
      reduceBeforeFirstRecv_ = hipHostReduce(
          streams_,
    ...
    if (!devicePtrsForFirstSend_.empty()) {
      reduceBeforeFirstSend_ = hipDeviceReduce(
          streams_,
          devicePtrsForFirstSend_,
          scratchPtrForFirstSend_,
          fn_,
    ...
    if (!devicePtrsForFirstRecv_.empty()) {
      reduceBeforeFirstRecv_ = hipDeviceReduce(
          streams_,
          devicePtrsForFirstRecv_,
          scratchPtrForFirstRecv_,
          fn_,
  ...
  for (int i = 0; i < stepsWithinBlock_; i++) {
    ...
    if (numElementsInBcast * sizeof(T) < kOnDeviceThreshold) {
      broadcastOps_.push_back(hipHostBroadcast(
          streams_,
      ...
      broadcastOps_.push_back(hipDeviceBroadcast(
          streams_,
          devicePtrsForBroadcast_[i],
          scratchPtrForBroadcast_[i],
          0,
...
#define INSTANTIATE_TEMPLATE(T)                                         \
  template class HipAllreduceHalvingDoubling<T, HipHostWorkspace<T>>; \
  template class HipAllreduceHalvingDoubling<T, HipDeviceWorkspace<T>>;
