  ...
  if (in[0] != out[0]) {
    if (in.size() == 1) {
      reduceInputs = [&](size_t offset, size_t length, size_t /* unused */) {
        memcpy(
            (uint8_t*) out[0]->ptr + offset,
            (uint8_t*) in[0]->ptr + offset,
            length);
      };
    } else {
      reduceInputs = [&](size_t offset, size_t length, size_t elementSize) {
        opts.reduce(
            (uint8_t*)out[0]->ptr + offset,
            (uint8_t*)in[0]->ptr + offset,
            (uint8_t*)in[1]->ptr + offset,
            length / elementSize);
        for (size_t i = 2; i < in.size(); i++) {
          opts.reduce(
              (uint8_t*)out[0]->ptr + offset,
              (uint8_t*)out[0]->ptr + offset,
              (uint8_t*)in[i]->ptr + offset,
              length / elementSize);
        }
      };
    }
  ...
  for (auto i = 0; i < numSegments; i++) {
    if (i >= 2) {
      ...
      if (prev.recvLength > 0) {
        ...
        opts.reduce(
            (uint8_t*)out[0]->ptr + prev.recvOffset,
            (uint8_t*)out[0]->ptr + prev.recvOffset,
            (uint8_t*)tmp->ptr + segmentOffset[i & 0x1],
