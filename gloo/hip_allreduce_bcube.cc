...
HipAllreduceBcube<T, W>::HipAllreduceBcube(
  ...
  for (auto i = 0; i < ptrs.size(); i++) {
    auto ptr = HipDevicePointer<T>::create(ptrs[i], totalNumElems_);
    if (newStream) {
...
template <typename T, typename W>
void HipAllreduceBcube<T, W>::run() {
  HipDeviceGuard guard;
  HipStream& stream = *scratchStream_;
...
template <typename T, typename W>
int HipAllreduceBcube<T, W>::computeSteps(int nodes, int peers) {
...
template <typename T, typename W>
bool HipAllreduceBcube<T, W>::printCheck(int /* rank */) {
  return false;
}

template <typename T, typename W>
void HipAllreduceBcube<T, W>::printBreak(T* p, int x) {
  if (0 == x % wordsPerLine) {
    std::cout << std::endl
...
template <typename T, typename W>
void HipAllreduceBcube<T, W>::printElems(T* p, int count, int start) {
...
template <typename T, typename W>
void HipAllreduceBcube<T, W>::printStageBuffer(const std::string& msg) {
...
template <typename T, typename W>
void HipAllreduceBcube<T, W>::printStepBuffer(
...
template <typename T, typename W>
const std::vector<int>& HipAllreduceBcube<T, W>::getPeersPerStep(
    int rank,
    int step) {
...
template <typename T, typename W>
int HipAllreduceBcube<T, W>::getNumElemsPerStep(int rank, int step) {
  return allNodes_[rank].getNumElemsPerStep(step);
}

template <typename T, typename W>
int HipAllreduceBcube<T, W>::getPtrOffsetPerStep(int rank, int step) {
  return allNodes_[rank].getPtrOffsetPerStep(step);
}

template <typename T, typename W>
void HipAllreduceBcube<T, W>::createNodes() {
...
template <typename T, typename W>
void HipAllreduceBcube<T, W>::updateGroupNodes(
    int step,
    const hip::allreduce::bcube::Group& group) {
  ...
  for (int i = 0; i < peersSz; ++i) {
    hip::allreduce::bcube::Node& node = allNodes_[peers[i]];
...
template <typename T, typename W>
void HipAllreduceBcube<T, W>::setupNodes() {
  ...
  int peerDistance = 1;
  for (int step = 0; step < steps_; ++step) {
    std::vector<hip::allreduce::bcube::Group> groups;
    // Iterate over all the nodes to identify the first node of each group
    for (int rank = 0; rank < nodes_; ++rank) {
      const hip::allreduce::bcube::Node& firstNode = allNodes_[rank];
...
template <typename T, typename W>
template <typename U>
void HipAllreduceBcube<T, W>::init(
  ...
  if (bytes_ < kOnDeviceThreshold) {
    localReduceOp_ =
        hipHostReduce(streams_, devicePtrs_, scratch_, fn_, 0, totalNumElems_);
    localBroadcastOp_ =
        hipHostBroadcast(streams_, devicePtrs_, scratch_, 0, totalNumElems_);
  } else {
    localReduceOp_ = hipDeviceReduce(
        streams_, devicePtrs_, scratch_, fn_, 0, totalNumElems_);
    localBroadcastOp_ =
        hipDeviceBroadcast(streams_, devicePtrs_, scratch_, 0, totalNumElems_);
  }
}
...
void HipAllreduceBcube<T, W>::init(
  ...
  auto index = findHipDevicePointerClosestToDevice(
      devicePtrs_, this->context_->getDevice());
  scratch_ = HipDevicePointer<T>::create(devicePtrs_[index]);
...
#define INSTANTIATE_TEMPLATE(T)                               \
  template class HipAllreduceBcube<T, HipHostWorkspace<T>>; \
  template class HipAllreduceBcube<T, HipDeviceWorkspace<T>>;
