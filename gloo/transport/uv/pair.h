...
class Pair : public ::gloo::transport::Pair {
 ...
 private:
  ...
  // Address that the remote side of this pair can connect to.
  Address addr_;

  // Handle of the connection, if the connection was made successfully.
  std::shared_ptr<libuv::TCP> handle_;

  // Error state of the handle, if set.
  // This is convertible to uvw::ErrorEvent to turn it into an error message.
  int errno_;
