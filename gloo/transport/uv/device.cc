...
void Device::connectAsListener(
  ...
  defer([=] {
    ...
    if (it != pendingConnections_.end()) {
      auto pendingConnection = std::move(it->second);
      pendingConnections_.erase(it);
      fn(std::move(pendingConnection), libuv::ErrorEvent(0));
    } else {
      // TODO(pietern): Handle timeout.
      pendingConnectCallbacks_[local.getSeq()] = std::move(fn);
    }
  });
}
...
void Device::connectAsListenerCallback(
...

  {
    ...
    if (it != pendingConnectCallbacks_.end()) {
      ...
      fn(std::move(handle), libuv::ErrorEvent(0));
    } else {
      pendingConnections_.emplace(seq, std::move(handle));
    }
  }
}
