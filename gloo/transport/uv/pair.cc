...
Pair::Pair(
    ...
    : context_(context),
      ...
      timeout_(timeout),
      errno_(0) {
  // The address for this pair is represented by the sockaddr of the listening
  // socket and a unique identifier. The peer connects to the address and then
  // writes the identifier to the new connection. The device instance The
  // address a peer must connect to encapsulates both the sockaddr and an
...
Pair::~Pair() {
  ...
  if (handle_) {
    // The handle is still around and must be closed.
    ...
    // Explicitly capture the handle to force a copy of the
    // shared_ptr. The `handle_` member field may be reset by an event
    // listener before this deferred call executes.
    auto handle = handle_;
    device_->defer([=, &closed] {
      handle->close();
      {
        std::unique_lock<std::mutex> lock(mutex_);
        closed = true;
...
// Called if the handle is closed by the peer.
//
// Threading: called from event loop thread.
