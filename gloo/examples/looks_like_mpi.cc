...
int MPI_Allreduce(
  ...
  opts.setInput(const_cast<int*>(reinterpret_cast<const int*>(sendbuf)), count);
  opts.setOutput(static_cast<int*>(recvbuf), count);
  typedef void (*ReduceFunction)(void*, const void*, const void*, size_t);
  opts.setReduceFunction(static_cast<ReduceFunction>(&gloo::sum<int>));
  gloo::allreduce(opts);
  return 0;
}
...
int MPI_Recv(
    ...
    int source,
    int tag,
    MPI_Comm comm) { // NOLINT unused parameter
