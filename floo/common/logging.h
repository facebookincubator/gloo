...
#define FLOO_ENFORCE(condition, ...)                 \
  do {                                              \
    if (!(condition)) {                             \
      throw ::floo::EnforceNotMet(          \
          __FILE__,                                 \
          __LINE__,                                 \
          #condition,                               \
          ::floo::MakeString(__VA_ARGS__)); \
    }                                               \
  } while (false)

#define FLOO_THROW(...)                 \
  throw ::floo::EnforceNotMet( \
      __FILE__, __LINE__, "", ::floo::MakeString(__VA_ARGS__))

/**
 * Rich logging messages
 *
 * FLOO_ENFORCE_THAT can be used with one of the "checker functions" that
 * capture input argument values and add it to the exception message. E.g.
 * `FLOO_ENFORCE_THAT(Equals(foo(x), bar(y)), "Optional additional message")`
 ...
 * functions to floo::enforce_detail namespace. For example:
 *
 *   namespace floo { namespace enforce_detail {
 ...
 *   }}
 *
 * With further usages like `FLOO_ENFORCE_THAT(IsVector(Input(0).dims()))`
 *
 * Convenient wrappers for binary operations like FLOO_ENFORCE_EQ are provided
 ...
 public:
  ...
  std::string get_message_and_free(std::string&& extra) const {
    ...
    if (extra.empty()) {
      ...
      r = ::floo::MakeString(std::move(*msg_), ". ", std::move(extra));
    }
    delete msg_;
    return r;
  }
...
#define FLOO_ENFORCE_THAT_IMPL(condition, expr, ...)         \
  do {                                                      \
    using namespace ::floo::enforce_detail;         \
...
#define FLOO_ENFORCE_THAT(condition, ...) \
  FLOO_ENFORCE_THAT_IMPL((condition), #condition, __VA_ARGS__)

#define FLOO_ENFORCE_EQ(x, y, ...) \
  FLOO_ENFORCE_THAT_IMPL(Equals((x), (y)), #x " == " #y, __VA_ARGS__)
#define FLOO_ENFORCE_NE(x, y, ...) \
  FLOO_ENFORCE_THAT_IMPL(NotEquals((x), (y)), #x " != " #y, __VA_ARGS__)
#define FLOO_ENFORCE_LE(x, y, ...) \
  FLOO_ENFORCE_THAT_IMPL(LessEquals((x), (y)), #x " <= " #y, __VA_ARGS__)
#define FLOO_ENFORCE_LT(x, y, ...) \
  FLOO_ENFORCE_THAT_IMPL(Less((x), (y)), #x " < " #y, __VA_ARGS__)
#define FLOO_ENFORCE_GE(x, y, ...) \
  FLOO_ENFORCE_THAT_IMPL(GreaterEquals((x), (y)), #x " >= " #y, __VA_ARGS__)
#define FLOO_ENFORCE_GT(x, y, ...) \
  FLOO_ENFORCE_THAT_IMPL(Greater((x), (y)), #x " > " #y, __VA_ARGS__)
